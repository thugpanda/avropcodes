...
word InstructionSniffer::catalogInstruction(word __pgmRead) {
    byte    __pgmReadByte   =   __pgmRead>>8;

    for(int i=0; i<3; i++) {
        if(((__pgmRead & doubleWordInstructionCode[i])^doubleWordInstructionCode[i]) == 0) {
            return i;
        }
    }

    for(int i=0; i<30; i++) {
        if((__pgmRead^fixedInstructionsCode[i]) == 0) {
            return i+3;
        }
    }
    
    for(int i=0; i<60; i++) {
        if(((__pgmRead & singleWordInstructionCode[i]) ^ singleWordInstructionCode[i]) == 0) {
            return i+33;
        }
    }

    for(int i=0; i<40; i++) {
        if(((__pgmReadByte & oneByteInstructions[i]) ^ oneByteInstructions[i]) == 0 ) {
            return i+93;
        }
    }
...
word InstructionSniffer::findInstruction(word __pFlash) {
    ...
    return catalogInstruction(pgmRead);
}
